...
import * as MidiNoteRangeFilter from '../components/MidiNoteRangeFilter';
...
describe('MIDIProvider', () => {
  ...
  it('Test Case 1 (Audio Interception) & Test Case 2 (Release Mapping): filters notes before attack and release', async () => {
    const mockMidiAccess = {
      inputs: new Map([['input-1', mockMidiInput1]]),
      outputs: new Map(),
      onstatechange: null,
    ...
    const attackSpy = vi.spyOn(audioEngine, 'triggerAttack');
    const releaseSpy = vi.spyOn(audioEngine, 'triggerRelease');
    const filterSpy = vi.spyOn(midiProcessing, 'applyNoteFilter');
    ...
    act(() => {
      capturedContext.setFilterMode('block');
      capturedContext.setFilterRange([0, 127]);
      capturedContext.setSelectedNotes([-5, 60]);
      capturedContext.updateButtonConfig('CUSTOM' as any, { midiNote: 36, stepSize: 1 });
    });

    // Simulate Note On for MIDI note 36 (mapped to CUSTOM)
    act(() => {
    ...
    expect(filterSpy).toHaveBeenCalledWith([-5, 60], 'block', [0, 127]);
    expect(attackSpy).toHaveBeenCalledWith([60], 100 / 127);

    // Simulate Note Off for MIDI note 36
    act(() => {
      const mockMidiMessageDetailOff = {
        data: new Uint8Array([0x80, 36, 0]),
        timestamp: 12346,
        input: mockMidiInput1,
      };
      window.dispatchEvent(new window.CustomEvent('MIDI_MESSAGE_RECEIVED', { detail: mockMidiMessageDetailOff }));
    });

    expect(releaseSpy).toHaveBeenCalledWith([60]);
  });

  it('Phase 1 Checkpoint: Test Case 1 (Audio Interception via State)', async () => {
    const mockMidiAccess = {
      inputs: new Map([['input-1', mockMidiInput1]]),
      outputs: new Map(),
      onstatechange: null,
    ...
    act(() => {
      capturedContext.setFilterMode('block');
      capturedContext.setFilterRange([60, 72]);
      capturedContext.setSelectedNotes([50, 64, 80]);
      capturedContext.updateButtonConfig('CUSTOM_P1' as any, { midiNote: 40, stepSize: 1 });
    });

    act(() => {
      const mockMidiMessageDetail = {
        data: new Uint8Array([0x90, 40, 100]),
        timestamp: 1001,
        input: mockMidiInput1,
      };
      window.dispatchEvent(new window.CustomEvent('MIDI_MESSAGE_RECEIVED', { detail: mockMidiMessageDetail }));
    ...
    expect(attackSpy).toHaveBeenCalledWith([64], 100 / 127);
  });

  it('Phase 2 Checkpoint: Test Case 1 (Visual Bridge Invocation)', async () => {
    const mockMidiAccess = {
      inputs: new Map([['input-1', mockMidiInput1]]),
      outputs: new Map(),
      onstatechange: null,
    ...
    const visualSpy = vi.spyOn(MidiNoteRangeFilter, 'updateKeyVisuals128');
    ...
    act(() => {
      capturedContext.setFilterMode('block');
      capturedContext.setFilterRange([60, 72]);
      capturedContext.setSelectedNotes([60, 64]);
      capturedContext.updateButtonConfig('CUSTOM_P2' as any, { midiNote: 42, stepSize: 1 });
    });

    act(() => {
      const mockMidiMessageDetail = {
        data: new Uint8Array([0x90, 42, 100]),
        timestamp: 1002,
        input: mockMidiInput1,
      };
      window.dispatchEvent(new window.CustomEvent('MIDI_MESSAGE_RECEIVED', { detail: mockMidiMessageDetail }));
    ...
    expect(visualSpy).toHaveBeenCalledWith(60, '#3b82f6');
    expect(visualSpy).toHaveBeenCalledWith(64, '#3b82f6');

    act(() => {
      const mockMidiMessageDetailOff = {
        data: new Uint8Array([0x80, 42, 0]),
        timestamp: 1003,
        input: mockMidiInput1,
      };
      window.dispatchEvent(new window.CustomEvent('MIDI_MESSAGE_RECEIVED', { detail: mockMidiMessageDetailOff }));
    ...
    expect(visualSpy).toHaveBeenCalledWith(60, '');
    expect(visualSpy).toHaveBeenCalledWith(64, '');
  });

  it('PRP #145 Phase 1 Checkpoint: Test Case 1 (Upstream State Update)', async () => {
    const mockMidiAccess = {
      inputs: new Map([['input-1', mockMidiInput1]]),
      outputs: new Map(),
      onstatechange: null,
    ...
    act(() => {
      capturedContext.setFilterMode('wrap');
      capturedContext.setFilterRange([0, 127]);
      capturedContext.setSelectedNotes([-5, 60]);
      capturedContext.updateButtonConfig('CUSTOM_P1' as any, { midiNote: 44, stepSize: 1 });
    });
    ...
    expect(attackSpy).toHaveBeenCalledWith([123, 60], 100 / 127);
    expect(capturedContext.activeTransformationNotes.get(44)).toEqual([123, 60]);
  });

  it('PRP #145 Phase 2 Checkpoint: Test Case 1 (Modal Visuals)', async () => {
    const mockMidiAccess = {
      inputs: new Map([['input-1', mockMidiInput1]]),
      outputs: new Map(),
      onstatechange: null,
    ...
    act(() => {
      capturedContext.setFilterMode('block');
      capturedContext.setFilterRange([0, 127]);
      capturedContext.setSelectedNotes([12, 16]);
      capturedContext.updateButtonConfig('CUSTOM_P2' as any, { midiNote: 46, stepSize: 1 });
    });

    act(() => {
      const mockMidiMessageDetail = {
        data: new Uint8Array([0x90, 46, 100]),
        timestamp: 1005,
        input: mockMidiInput1,
      };
      window.dispatchEvent(new window.CustomEvent('MIDI_MESSAGE_RECEIVED', { detail: mockMidiMessageDetail }));
    ...
    expect(visualSpy).toHaveBeenCalledWith(12, '#3b82f6');
    expect(visualSpy).toHaveBeenCalledWith(16, '#3b82f6');
  });
});
