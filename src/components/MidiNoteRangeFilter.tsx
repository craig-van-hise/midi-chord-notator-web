...
const getNoteCenterX = (note: number): number => {
...
const getClosestNote = (x: number): number => {
  let minNote = 0;
  ...
  return (
    <div style={{ width: `${TOTAL_WIDTH}px`, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          ...
          {/* Thumbs */}
          <Thumb x={x1} value={value[0]} onPointerDown={(e) => handlePointerDown(e, 'min')} isDragging={draggingThumb === 'min'} />
          <Thumb x={x2} value={value[1]} onPointerDown={(e) => handlePointerDown(e, 'max')} isDragging={draggingThumb === 'max'} />
        </div>
    </div>
  );
};

const Thumb = ({ x, value, onPointerDown, isDragging }: { x: number, value: number, onPointerDown: (e: React.PointerEvent<HTMLDivElement>) => void, isDragging: boolean }) => {
  return (
    <div 
...
export const updateKeyVisuals128 = (note: number, color: string): void => {
    ...
    if (color) {
        ...
        el.style.boxShadow = `inset 0 -4px 8px rgba(0,0,0,0.2), 0 0 10px ${color}`;
...
export function MidiNoteRangeFilter({
  ...
  const [internalMode, setInternalMode] = useState<FilterMode>('block');
  const [internalRange, setInternalRange] = useState<[number, number]>([0, 127]);
