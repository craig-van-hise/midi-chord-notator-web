...
export const ArrowContextMenu: React.FC<ArrowMenuProps> = ({ 
  ...
  const isActionBtn = ['PLAY', 'HOME', 'UNDO', 'REDO'].includes(buttonId);
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center p-3 border-b-2 border-black bg-black text-white">
        ...
        <button onClick={onClose} className="hover:text-green-400 font-bold px-1">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="p-4 space-y-5">
        ...
        {!isActionBtn && (
          <div>
            <div className="flex justify-between mb-2 items-center">
              <label className="text-black font-extrabold uppercase text-xs">Step Size</label>
            ...
            />
            <div className="flex justify-between text-[10px] text-gray-900 font-bold mt-1">
              <span>1</span>
              <span>12</span>
            </div>
          </div>
        ...
        <div className="border border-black p-2 bg-gray-50">
          <div className="flex justify-between border-b border-dashed border-gray-400 pb-1 mb-1">
            <span className="text-black font-extrabold text-xs">MIDI CH</span>
            <span className="font-bold">{config.midiChannel}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-black font-extrabold text-xs">NOTE #</span>
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col">
            ...
            <span className="text-[10px] text-black font-extrabold">{settings.listenMode ? 'ON' : 'OFF'}</span>
        </div>
      </button>
    </div>
  );
